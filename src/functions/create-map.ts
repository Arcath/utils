interface Map<T>{
  ...
  setRange(minX: number, maxX: number, minY: number, maxY: number, value: T): void
 ...
 */
export const createMap = <T>(maxX: number, maxY: number, initialValue: T, minX = 0, minY = 0): Map<T> => {
  const map: T[][] = []
